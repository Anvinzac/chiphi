-- Track how often each catalog ingredient appears on order lines.
...
-- Keep order_count in sync with order_items insert/delete (including CASCADE).
...
BEGIN
  ...
  SET
    order_count = GREATEST(0, oi.order_count + v_delta),
    updated_at = now()
...
-- Backfill from existing order history (line appearances).
UPDATE public.order_ingredients oi
